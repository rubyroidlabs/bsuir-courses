  ...
  def start
    Telegram::Bot::Client.run(@token) do |bot|
      bot.listen do |message|
        status = if message.text.split.size == (1 || 0)
                   clarify_full_name(bot, message)
                 elsif message.text.scan(/[А-Яа-я]/).any?
                   clarify_language(bot, message)
                 else
                   Orientation.by_full_name(message.text)
                 end
        if status.nil?
