  ...
  bot.listen do |message|
    case message.text
      ...
      when '/stop'
        ...
        if parser.search_in_gay_actors || parser.search_in_bisexual_actors || parser.search_in_lesbian_actors
          bot.api.sendMessage(chat_id: message.chat.id, text: parser.print)
        else
          bot.api.sendMessage(chat_id: message.chat.id, text: 'This person is not in this list')
        end
    end
  end
end