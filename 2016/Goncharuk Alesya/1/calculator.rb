def reset_bits (value, number_bits)
   num_resets = 0
   n = 0
   while (value != 0 && num_resets < number_bits)
       if(is_set_bit value, 2 ** n)
          num_resets += 1
          value = value ^ 2 ** n
       end
       n += 1
...
def is_set_bit (value, number_marker)
...
puts "Начните ввовдить своё выражение. "
...
quantity_numbers += 1;
stack_numbers = Array.new
stack_symbols = Array.new
stack_numbers.push(symbol)
if(flag_input_item == true)
  loop do
     symbol = gets.chomp
        if symbol.to_i != 0
             quantity_numbers += 1
